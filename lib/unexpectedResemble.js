...
module.exports = {
  ...
  installInto(expect) {
    ...
    expect.exportAssertion(
      '<string|binaryArray> to resemble <any+>',
      function (expect, subject, otherImage, value) {
              ...
              ['ignoreColors', 'ignoreAntialiasing'].forEach((optionName) => {
                if (optionName in value) {
                  ...
                  if (Array.isArray(options.ignore)) {
                    options.ignore.push(ignoreValue);
                  } else if (options.ignore !== undefined) {
                    options.ignore = [options.ignore, ignoreValue];
                  } else {
                    options.ignore = ignoreValue;
                  }
